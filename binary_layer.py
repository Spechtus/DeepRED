...
from tensorflow.python.ops import standard_ops, nn, variable_scope, math_ops, control_flow_ops, array_ops
...
class Dense_BinaryLayer(tf.compat.v1.layers.Dense):
    ...
    def build(self, input_shape):
        ...
        self.kernel_initializer = tf.compat.v1.random_uniform_initializer(-self.H, self.H)
        ...
        self.b_kernel = self.add_variable('binary_weight',
                                    shape=[input_shape[-1], self.units],
                                    initializer=tf.compat.v1.random_uniform_initializer(-self.H, self.H),
                                    regularizer=None,
                                    constraint=None,
                                    dtype=self.dtype,
                                    trainable=False)
        ...
        tf.compat.v1.add_to_collection(self.name + '_binary', self.kernel)  # layer-wise group
        tf.compat.v1.add_to_collection('binary', self.kernel)               # global group
    ...
    def call(self, inputs):
        ...
        if self.activation is not None:
            print("activation(output)",self.activation(outputs))
...
class Conv2D_BinaryLayer(tf.compat.v1.layers.Conv2D):
    '''
    ...
    def build(self, input_shape):
        ...
        tf.compat.v1.add_to_collection(self.name + '_binary', self.kernel)  # layer-wise group
        tf.compat.v1.add_to_collection('binary', self.kernel)
...
def compute_grads(loss, opt):
    ...
    for layer in layers:
        ...
        params = tf.compat.v1.get_collection(layer.name + "_binary")
        if params:
            # print(params[0].name)
    ...
    #print("grad_list:",grads_list)
    #print("update_weights",update_weights)
