...
def retrain_network(data, model_name, new_model_name, hidden_nodes, iterations, batch_size=0):
	...
	def logfunc(x, x2):
		return tf.multiply( x, tf.math.log(tf.divide(x,x2)))
	...
	rho_hat[0] = tf.divide(tf.reduce_sum(input_tensor=tf.abs(A_train[0]),axis=0),tf.constant(float(batch_size)))
	divergance[0] = tf.reduce_sum(input_tensor=KL_Div(RHO, rho_hat[0]))

	for i in range(1,layers-1):
		A_train[i] = tf.tanh(tf.matmul(tf.nn.dropout(A_train[i-1], 1 - (keep_prob)), masks[i]*W[i]) + B[i])
		A_test[i] = tf.tanh(tf.matmul(A_test[i-1], masks[i]*W[i]) + B[i])
		rho_hat[i] = tf.divide(tf.reduce_sum(input_tensor=tf.abs(A_train[i]),axis=0),tf.constant(float(batch_size)))
		divergance[i] = tf.reduce_sum(input_tensor=KL_Div(RHO, rho_hat[i]))
	...
	loss = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits, Y_train)) #ADDED v2 in TF 2.0
	cost_sparse = tf.reduce_sum(input_tensor=divergance)
	cost = tf.add(loss , tf.multiply(BETA, cost_sparse))
	train_step = tf.compat.v1.train.AdamOptimizer().minimize(cost)
...
def keep_training_wsp_polarize(data, bet, model_name, new_model_name, hidden_nodes, iterations, accuracy_decrease = 0.01, indexes = [], to_prune=[], together = 1, batch_size = 0, max_non_improvements=2, max_runs=5):
	...
	def logfunc(x, x2):
		return tf.mul( x, tf.math.log(tf.compat.v1.div(x,x2)))
	...
	rho_hat[0] = tf.compat.v1.div(tf.reduce_sum(input_tensor=tf.abs(A_train[0]),axis=0),tf.constant(float(batch_size)))
	divergance[0] = tf.reduce_sum(input_tensor=KL_Div(RHO, rho_hat[0]))
	
	for i in range(1,layers-1):
		A_train[i] = tf.tanh(tf.matmul(A_train[i-1], masks[i]*W[i]) + B[i])
		A_vali[i] = tf.tanh(tf.matmul(A_vali[i-1], masks[i]*W[i]) + B[i])
		rho_hat[i] = tf.compat.v1.div(tf.reduce_sum(input_tensor=tf.abs(A_train[i]),axis=0),tf.constant(float(batch_size)))
		divergance[i] = tf.reduce_sum(input_tensor=KL_Div(RHO, rho_hat[i]))
	...
	loss = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(Y_train)))
	cost_sparse = tf.reduce_sum(input_tensor=divergance)
