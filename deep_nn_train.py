...
def init_weights(shape, i):
	weights_name = 'W'
	weights_name += str(i)
	return tf.Variable(tf.random.uniform(shape, -1., 1.), name = weights_name)

def init_bias(size, i):
	bias_name = 'B'
	bias_name += str(i)
	return tf.Variable(tf.random.uniform([size], -1, 1), name = bias_name)
...
def train_network(data, model_name, hidden_nodes, iterations, function ='tanh', softmax=True, batch_size=0):
	...
	if data.num_vali == 0:
		x_vali, y_vali = data.get_test_x_y()
	...
	# Set inputs from [0,1] to [-1,1]
	#for i in x:
	#	i = i*2. - 1.
	#for i in x_test:
	#	i = i*2. - 1.
	
	#save train, vali and test in datapkl/
	with open('datapkl/'+model_name+'x_train.pkl', 'wb') as f:
		pickle.dump(x, f, pickle.HIGHEST_PROTOCOL)
	
	with open('datapkl/'+model_name+'y_train.pkl', 'wb') as f:
		pickle.dump(y, f, pickle.HIGHEST_PROTOCOL)

	with open('datapkl/'+model_name+'x_test.pkl', 'wb') as f:
		pickle.dump(x_test, f, pickle.HIGHEST_PROTOCOL)
	
	with open('datapkl/'+model_name+'y_test.pkl', 'wb') as f:
		pickle.dump(y_test, f, pickle.HIGHEST_PROTOCOL)

	with open('datapkl/'+model_name+'x_vali.pkl', 'wb') as f:
		pickle.dump(x_vali, f, pickle.HIGHEST_PROTOCOL)
	
	with open('datapkl/'+model_name+'y_vali.pkl', 'wb') as f:
		pickle.dump(y_vali, f, pickle.HIGHEST_PROTOCOL)

	X_train = tf.compat.v1.placeholder(tf.float32, shape=[batch_size,input_size])
	Y_train = tf.compat.v1.placeholder(tf.float32, shape=[batch_size,output_size])
	
	X_vali = tf.compat.v1.placeholder(tf.float32, shape=[len(x_vali),input_size])
	Y_vali = tf.compat.v1.placeholder(tf.float32, shape=[len(x_vali),output_size])

	X_test = tf.compat.v1.placeholder(tf.float32, shape=[len(x_test),input_size])
	...
	if function == 'tanh':
		...
		A_train[0] = tf.nn.relu(tf.matmul(X_train, masks[0]*W[0]) + B[0])
		A_vali[0] = tf.nn.relu(tf.matmul(X_vali, masks[0]*W[0]) + B[0])
		A_test[0] = tf.nn.relu(tf.matmul(X_test, masks[0]*W[0]) + B[0])
	else:
	...
	for i in range(1,layers-1):
		if function == 'tanh':
			...
			A_train[i] = tf.tanh(tf.matmul(tf.nn.dropout(A_train[i-1], 1 - (keep_prob)), masks[i]*W[i]) + B[i]) #ADDED 1 - (keep_prob) for TFv2
			A_vali[i] = tf.tanh(tf.matmul(A_vali[i-1], masks[i]*W[i]) + B[i])
			A_test[i] = tf.tanh(tf.matmul(A_test[i-1], masks[i]*W[i]) + B[i])
		elif function == 'relu':
			A_train[i] = tf.nn.relu(tf.matmul(tf.nn.dropout(A_train[i-1], 1 - (keep_prob)), masks[i]*W[i]) + B[i]) #ADDED 1 - (keep_prob) for TFv2
			A_vali[i] = tf.nn.relu(tf.matmul(A_vali[i-1], masks[i]*W[i]) + B[i])
			A_test[i] = tf.nn.relu(tf.matmul(A_test[i-1], masks[i]*W[i]) + B[i])
		else:
	...
	if softmax:
		...
		logits_vali = tf.matmul(A_vali[layers-2], masks[layers-1]*W[layers-1]) + B[layers-1]
		logits_test = tf.matmul(A_test[layers-2], masks[layers-1]*W[layers-1]) + B[layers-1]
		A_train[layers-1] = tf.nn.softmax(logits)
		A_vali[layers-1] = tf.nn.softmax(logits_vali)
		A_test[layers-1] = tf.nn.softmax(logits_test)
		Hypothesis_train = A_train[layers-1]
		Hypothesis_vali = A_vali[layers-1]
		Hypothesis_test = A_test[layers-1]
		loss = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(Y_train),logits=logits))
		loss_vali = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(Y_vali),logits=logits_vali)) #ADD: added logits=logits for TFv2
		loss_test = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(Y_test),logits=logits_test)) #ADD: added logits=logits for TFv2
	else:
		if function == 'tanh':
			...
			A_train[layers-1] = tf.nn.relu(tf.matmul(A_train[layers-2], masks[layers-1]*W[layers-1]) + B[layers-1])
			A_vali[layers-1] = tf.nn.relu(tf.matmul(A_vali[layers-2], masks[layers-1]*W[layers-1]) + B[layers-1])
			A_test[layers-1] = tf.nn.relu(tf.matmul(A_test[layers-2], masks[layers-1]*W[layers-1]) + B[layers-1])
		else:
		...
		Hypothesis_vali = A_vali[layers-1]
		Hypothesis_test = A_test[layers-1]
		loss = tf.reduce_mean(input_tensor=tf.math.squared_difference(Hypothesis_train, Y_train))
		loss_vali = tf.reduce_mean(input_tensor=tf.math.squared_difference(Hypothesis_vali, Y_vali))
		loss_test = tf.reduce_mean(input_tensor=tf.math.squared_difference(Hypothesis_test, Y_test))
	print("Hypothesis train",Hypothesis_train)
	print("Hypothesis vali",Hypothesis_vali)
	print("Hypothesis test",Hypothesis_test)
	...
	for i in range(iterations):
		...
		if (i % (iterations/10) == 0) or (i + 1 == iterations):
			...
			print('TRAINCOST ', sess.run(loss, feed_dict={X_train: x_train, Y_train: y_train, keep_prob: rate})) #ADDED rate for TF v2
			h_train = sess.run(Hypothesis_train, feed_dict={X_train: x_train, keep_prob: rate}) #ADDED rate for TF v2
			print('TRAIN ACCURACY', accuracy(x_train, y_train, h_train))

			print('VALICOST ', sess.run(loss_vali, feed_dict={X_vali: x_vali, Y_vali: y_vali, keep_prob: rate})) #ADDED rate for TF v2
			h_vali = sess.run(Hypothesis_vali, feed_dict={X_vali: x_vali, keep_prob: rate}) #ADDED rate for TF v2
			print('VALI ACCURACY', accuracy(x_vali, y_vali, h_vali))

			print('TESTCOST ', sess.run(loss_test, feed_dict={X_test: x_test, Y_test: y_test, keep_prob: rate})) #ADDED rate for TF v2
			h_test = sess.run(Hypothesis_test, feed_dict={X_test: x_test, keep_prob: rate}) #ADDED rate for TF v2
			print('TEST ACCURACY', accuracy(x_test, y_test, h_test))
		#h_vali = sess.run(Hypothesis_vali, feed_dict={X_vali: x_vali, keep_prob: rate})
		#if accuracy(x_vali, y_vali, h_vali) == 1:
		#	print('Final Epoch:', i)
		#	print('Final TRAINCOST ', sess.run(loss, feed_dict={X_train: x_train, Y_train: y_train, keep_prob: rate})) #ADDED rate for TF v2
		#	h_train = sess.run(Hypothesis_train, feed_dict={X_train: x_train, keep_prob: rate}) #ADDED rate for TF v2
		#	print('Final TRAIN ACCURACY', accuracy(x_train, y_train, h_train))
		#	print('Final VALICOST ', sess.run(loss_vali, feed_dict={X_vali: x_vali, Y_vali: y_vali, keep_prob: rate}))
		#	break
	...
	h_vali = sess.run(Hypothesis_vali, feed_dict={X_vali: x_vali, keep_prob: rate}) 
	return accuracy(x_vali, y_vali, h_vali)
...
def execute_network(data, model_name, hidden_nodes, function = 'tanh', softmax=True):
	...
	if function == 'tanh':
		...
		A_vali[0] = tf.nn.relu(tf.matmul(X_vali, W[0]) + B[0])
		A_test[0] = tf.nn.relu(tf.matmul(X_test, W[0]) + B[0])
	elif function == 'bin':
		A_train[0] = tf.Sign(tf.matmul(X_train, W[0]) + B[0])
		A_vali[0] = tf.Sign(tf.matmul(X_vali, W[0]) + B[0])
		A_test[0] = tf.Sign(tf.matmul(X_test, W[0]) + B[0])
	else:
	...
	for i in range(1,layers-1):
		if function == 'tanh':
			...
			A_vali[i] = tf.nn.relu(tf.matmul(A_vali[i-1], W[i]) + B[i])
			A_test[i] = tf.nn.relu(tf.matmul(A_test[i-1], W[i]) + B[i])
		elif function == 'bin':
			A_train[i] = tf.Sign(tf.matmul(A_train[i-1], W[i]) + B[i])
			A_vali[i] = tf.Sign(tf.matmul(A_vali[i-1], W[i]) + B[i])
			A_test[i] = tf.Sign(tf.matmul(A_test[i-1], W[i]) + B[i])
		else:
	...
	if softmax:
		...
		if function == 'tanh':
			...
			A_vali[layers-1] = tf.nn.relu(tf.matmul(A_vali[layers-2], W[layers-1]) + B[layers-1])
			A_test[layers-1] = tf.nn.relu(tf.matmul(A_test[layers-2], W[layers-1]) + B[layers-1])
		elif function == 'bin':
			A_train[layers-1] = tf.Sign(tf.matmul(A_train[layers-2], W[layers-1]) + B[layers-1])
			A_vali[layers-1] = tf.Sign(tf.matmul(A_vali[layers-2], W[layers-1]) + B[layers-1])
			A_test[layers-1] = tf.Sign(tf.matmul(A_test[layers-2], W[layers-1]) + B[layers-1])
		else:
	...
	activation_values_vali[layers-1] = np.asarray(sess.run(Hypothesis_vali, feed_dict={X_vali: x_vali}))
	activation_values_test[layers-1] = np.asarray(sess.run(Hypothesis_test, feed_dict={X_test: x_test}))
...
def weight_sparseness_pruning(data, model_name, new_model_name, hidden_nodes, iterations, function = 'tanh', softmax = True, accuracy_decrease = 0.01, indexes = [], to_prune=[], together = 1, batch_size = 0, max_non_improvements=2, max_runs=5):
	...
	W[0] = init_weights([input_size, hidden_nodes[0]], 0)
	masks[0] = tf.compat.v1.placeholder(tf.float32, shape=[input_size, hidden_nodes[0]])
	W[layers-1] = init_weights([hidden_nodes[layers-2], output_size], layers-1)
	masks[layers-1] = tf.compat.v1.placeholder(tf.float32, shape=[hidden_nodes[layers-2], output_size])
	B[0] = init_bias(hidden_nodes[0], 0)
	B[layers-1] = init_bias(output_size, layers-1)
	...
	for i in range(layers-2):
		W[i+1] = init_weights([hidden_nodes[i], hidden_nodes[i+1]], i+1)
		B[i+1] = init_bias(hidden_nodes[i+1], i+1)
		masks[i+1] = tf.compat.v1.placeholder(tf.float32, shape=[hidden_nodes[i], hidden_nodes[i+1]])
	...
	if softmax:
		...
		Hypothesis_train = A_train[layers-1]
		Hypothesis_vali = A_vali[layers-1]
		loss = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits, Y_train))
	else:
		if function == 'tanh':
			A_train[layers-1] = tf.tanh(tf.matmul(A_train[layers-2], masks[layers-1]*W[layers-1]) + B[layers-1])
			A_vali[layers-1] = tf.tanh(tf.matmul(A_vali[layers-2], masks[layers-1]*W[layers-1]) + B[layers-1])
...
def rexren_input_prune(data, model_name, new_model_name, hidden_nodes, function, softmax, max_accuracy_decrease = 5, per_threshold = True):
	...
	W[0] = init_weights([input_size, hidden_nodes[0]], 0)
	mask = tf.compat.v1.placeholder(tf.float32, shape=[input_size, hidden_nodes[0]])
	W[layers-1] = init_weights([hidden_nodes[layers-2], output_size], layers-1)
	B[0] = init_bias(hidden_nodes[0], 0)
	B[layers-1] = init_bias(output_size, layers-1)
