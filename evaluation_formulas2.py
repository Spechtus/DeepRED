...
def network_accuracy(output_char, data, binaryExtraction):
	...
	subtrain=data.train_indexes
	
	n_trainexamples = len(subtrain)
	for i in subtrain + data.vali_indexes + data.test_indexes:
		...
		cntr += 1
	if n_trainexamples > 0:
		train_accuracy = float(correct_predictions_train / n_trainexamples)
	else:
		train_accuracy = 1
	if data.num_vali > 0:
...
def network_confusionmatrix(output_char, data, binaryExtraction):
	...
	subtrain= data.train_indexes[49:] 
...
def network_recall(output_char, data, binaryExtraction):
	...
	subtrain= data.train_indexes[:250] + data.train_indexes[299:] 
...
def class_accuracy(data, dnf, target_class_index=1, t_v = True, tr = False, v = False, te = False, binaryExtraction=False):
	...
	subtrain= data.get_train_obs()
	subtrain= subtrain[:300]
	...
	if t_v:
		...
		result.append(float(consistent)/n_examples)
	if tr:
		n_examples = len(subtrain)
		consistent = sum([1 if ((target_class_index == sign(e.class_value)) == e.fulfills_dnf(dnf)) else 0 for e in subtrain])
		print(consistent)
		print(n_examples)
...
def class_confusionmatrix(data, dnf, target_class_index=1, t_v = True, tr = False, v = False, te = False, binaryExtraction=False):
	...
	if t_v:
		...
		if tp+fn > 0:
			...
			recalls.append(1)
	if tr:
		tp = sum([1 if (target_class_index == sign(e.class_value)) and e.fulfills_dnf(dnf) else 0 for e in subtrain])
		fp = sum([1 if (target_class_index != sign(e.class_value)) and e.fulfills_dnf(dnf) else 0 for e in subtrain])
		fn = sum([1 if (target_class_index == sign(e.class_value)) and (not e.fulfills_dnf(dnf)) else 0 for e in subtrain])
		tn = sum([1 if (target_class_index != sign(e.class_value)) and (not e.fulfills_dnf(dnf)) else 0 for e in subtrain])
...
def confusionmatrix_of_dnf(data, key, dnf, t_v = True, tr = False, v = False, te = True):
	...
	if t_v:
		...
		if tp+fn > 0:
			...
			recalls.append(1)
	if tr:
		tp = sum([1 for e in subtrain if e.fulfills_cond(key) and e.fulfills_dnf(dnf)])
		fp = sum([1 for e in subtrain if (not e.fulfills_cond(key)) and e.fulfills_dnf(dnf)])
		fn = sum([1 for e in subtrain if e.fulfills_cond(key) and (not e.fulfills_dnf(dnf))])
		tn = sum([1 for e in subtrain if (not e.fulfills_cond(key)) and (not e.fulfills_dnf(dnf))])
