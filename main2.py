...
def extract_model(dataset_name, split_name, model_name, hidden_nodes, 
	...
	min_set_size=1, dis_config=0, rft_pruning_config=2, rep_pruning_config=1, 
	...
	if os.path.exists('obj/BNN_' + dataset_name + '_' + split_name + '2.pkl'):
		BNN, data.example_cond_dict, data.dict_indexes = lr.load_BNN_ecd_indexes(dataset_name + '_' + split_name+'2')
		...
		lr.save_BNN_ecd_indexes(BNN, data.example_cond_dict, data.dict_indexes, dataset_name + '_' + split_name+'2')
		print('\nBuilt BNN')
	...
	if os.path.exists('obj/bio_' + dataset_name + '_' + split_name + '2.pkl'):
		bio = lr.load_bio(dataset_name + '_' + split_name+'2')
		print('\nLoaded bio')
	else:
		t= time.time()
		bio = r.get_bio(BNN, output_condition, data.example_cond_dict, data.dict_indexes, with_data = rep_pruning_config, data=data)
		lr.save_bio(bio, dataset_name + '_' + split_name+'2')
		print('\nBuilt bio')
