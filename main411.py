...
def extract_model(dataset_name, split_name, model_name, hidden_nodes, 
	...
	if binary:
		...
		act_train, act_vali, act_test, weights = lr.load_bin_act_and_weights(model_name+c)
	...
	if os.path.exists('obj/BNN_' + dataset_name + '_' + split_name +c+'.pkl'):
		BNN, data.example_cond_dict, data.dict_indexes = lr.load_BNN_ecd_indexes(dataset_name + '_' + split_name+c)
		...
		lr.save_BNN_ecd_indexes(BNN, data.example_cond_dict, data.dict_indexes, dataset_name + '_' + split_name+c)
		print('\nBuilt BNN')
	...
	if os.path.exists('obj/bio_' + dataset_name + '_' + split_name + c + '.pkl'):
		bio = lr.load_bio(dataset_name + '_' + split_name+c)
		print('\nLoaded bio')
	else:
		t= time.time()
		bio = r.get_bio(BNN, output_condition, data.example_cond_dict, data.dict_indexes, with_data = rep_pruning_config, data=data)
		lr.save_bio(bio, dataset_name + '_' + split_name+c)
		print('\nBuilt bio')
